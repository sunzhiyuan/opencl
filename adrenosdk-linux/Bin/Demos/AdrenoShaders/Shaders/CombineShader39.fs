...
uniform sampler2D g_StreakTexture1;
uniform sampler2D g_StreakTexture2;
...
void main()
{
    ...
    vec4 Streak1 = texture2D( g_StreakTexture1, g_TexCoord );
    vec4 Streak2 = texture2D( g_StreakTexture2, g_TexCoord );
    ...
    vec4 sample_bloom = clamp( ScreenPixel + Streak1 + Streak2, 0.0, 1.0 );
