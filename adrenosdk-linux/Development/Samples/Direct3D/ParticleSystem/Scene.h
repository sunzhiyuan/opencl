...
struct PARTICLE
{
    ...
    FRMVECTOR2 m_vTexCoord;          // Texture coordinate for vertex
...
private:
    ...
    // Shader related data
    CFrmShaderProgramD3D m_ParticleShader;
    struct PointSpritesConstantBuffer
    {
        FRMMATRIX4X4 matModelViewProj;
        FLOAT32      fTime;
        FRMVECTOR3   Padding; // Pad to multiple of 16-bytes
    };
    PointSpritesConstantBuffer m_PointSpriteConstantBufferData;
    CFrmConstantBuffer*        m_pPointSpriteConstantBuffer;


    CFrmTexture*       m_pLogoTexture;
    CFrmTexture*       m_pParticleTexture;

    CFrmVertexBuffer*  m_pParticleVertexBuffer;
    CFrmIndexBuffer*   m_pParticleIndexBuffer;

    FRMMATRIX4X4       m_matProj;

    // Particle related data
    CParticleEmitter   m_ParticleEmitters[NUM_EMITTERS];
    PARTICLE           m_Particles[NUM_PARTICLES*4];
    ...
    Microsoft::WRL::ComPtr<ID3D11BlendState>         m_SrcAlphaBlendState;
