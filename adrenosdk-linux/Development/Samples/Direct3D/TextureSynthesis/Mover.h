...
public:
	Mover() : SpeedInc(10.0999f)
	{
		...
		InverseWorldMatrix = FrmMatrixLookAtRH( FRMVECTOR3( 0.0f, 0.0f, 1.0f ), FRMVECTOR3(0.0f, 0.0f, 0.0f), FRMVECTOR3(0.0f, 1.0f, 0.0f) );;
	}
	...
	void Update( FRM_INPUT& input, FLOAT32 dt)
	{
		...
		FRMMATRIX4X4 roty = FrmMatrixRotate( Spin.x, FRMVECTOR3(0.0f,1.0f,0.0f));
		FRMMATRIX4X4 rotx = FrmMatrixRotate( -Spin.y , FRMVECTOR3( roty.M(0,0), roty.M(1,0), roty.M(2,0)));//FRMVECTOR3(1.0f,0.0f,0.0f));
		...
		WorldPose += (CurVel * dt);
		InverseWorldMatrix = FrmMatrixTranslate( -WorldPose);
