...
function force_exit_timeout(){
        echo "$test_name Result: FAILED - TIMEOUT"
...
do
    ...
    if [[ "$test_name" =~ .*CLGLES ]]; then
       continue;
    fi
    log_file="$logpath"/"$test_name"_"$device_type".log""
    if [ -f $log_file ]; then
