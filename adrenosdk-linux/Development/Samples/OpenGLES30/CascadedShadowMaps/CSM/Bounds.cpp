...
void BoundSphere::expand(  BoundBox &bb) {
	if(bb.isValid()) {
		...
		if(isValid()) {
			FRMVECTOR3 v;
			v = FRMVECTOR3(min.x,min.y,min.z);
			expand(v);
			v = FRMVECTOR3(max.x,min.y,min.z);
			expand(v);
			v = FRMVECTOR3(min.x,max.y,min.z);
			expand(v);
			v = FRMVECTOR3(max.x,max.y,min.z);
			expand(v);
			v = FRMVECTOR3(min.x,min.y,max.z);
			expand(v);
			v = FRMVECTOR3(max.x,min.y,max.z);
			expand(v);
			v = FRMVECTOR3(min.x,max.y,max.z);
			expand(v);
			v = FRMVECTOR3(max.x,max.y,max.z);
			expand(v);
		} else {
			valid = 1;
			center = (min + max) * 0.5f;
			FRMVECTOR3 rvec = max - center;
			radius = FrmVector3Length(rvec);
		}
	}
}
/******************************************************************************\
