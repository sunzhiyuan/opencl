...
public:
	Mover() : SpeedInc(10.0999f)
	{
        ...
        FRMVECTOR3 zVec = FRMVECTOR3(0.0f, 0.0f, 1.0f);
        FRMVECTOR3 emptyVec = FRMVECTOR3(0.0f, 0.0f, 0.0f);
        FRMVECTOR3 upVec = FRMVECTOR3(0.0f, 1.0f, 0.0f);
		InverseWorldMatrix = FrmMatrixLookAtRH(zVec, emptyVec, upVec);
	}
	...
	void Update( FRM_INPUT& input, FLOAT32 dt)
	{
        ...
        FRMVECTOR3 upVec = FRMVECTOR3(0.0f, 1.0f, 0.0f);
		FRMMATRIX4X4 roty = FrmMatrixRotate( Spin.x, upVec);

        FRMVECTOR3 rotatedVec = FRMVECTOR3(roty.M(0,0), roty.M(1,0), roty.M(2,0));
		FRMMATRIX4X4 rotx = FrmMatrixRotate(-Spin.y , rotatedVec);
		...
		WorldPose += (CurVel * dt);
		InverseWorldMatrix = FrmMatrixTranslate(-WorldPose.x, -WorldPose.y, -WorldPose.z);
	...
	const FRMVECTOR3 GetPosition() { return -WorldPose; }
	const FRMVECTOR3 GetDirection()
	{
		return -FRMVECTOR3( InverseWorldMatrix.m[0][2], InverseWorldMatrix.m[1][2], InverseWorldMatrix.m[2][2] );
	}
