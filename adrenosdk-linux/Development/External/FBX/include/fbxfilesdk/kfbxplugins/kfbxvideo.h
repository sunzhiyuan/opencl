...
public:
    ...
    //@{
		...
		* Besides storing video clips, the KFbxVideo object can also store image sequences. This section contains
		...
		*/
		...
		typedef enum
		{
			...
			FullEvenOdd,    //!< Extract Fields and make full frame with each one beginning with Odd (60fps).
			FullOddEven     //!< Extract Fields and make full frame with each one beginning with Even (60fps).
		} EInterlaceMode;
...
public:
	...
	KFbxTypedProperty<fbxBool1>   ImageSequence;
	KFbxTypedProperty<fbxInteger1> ImageSequenceOffset;
