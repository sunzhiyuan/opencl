...
// Desc:
//--------------------------------------------------------------------------------------
VOID SimpleObject18::Update( FLOAT ElapsedTime, BOOL ShouldRotate )
{
    if( ShouldRotate )
...
// Desc: Constructor
//--------------------------------------------------------------------------------------
CSample18::CSample18( const CHAR* strName ) : CScene( strName )
{
    m_nWidth = 800;
    m_nHeight = 600;
}
...
// Desc: 
//--------------------------------------------------------------------------------------
BOOL CSample18::Initialize(CFrmFontGLES &m_Font, CFrmTexture* m_pLogoTexture)
{
    m_ShouldRotate = TRUE;

    // Load the packed resources
    CFrmPackedResourceGLES resource;
    if( FALSE == resource.LoadFromFile( "Demos/AdrenoShaders/Textures/18_Textures.pak" ) )
        return FALSE;
    ...
    // Set up the objects
    m_Object[0].DiffuseTexture = resource.GetTexture( "Leather1" );
    m_Object[0].BumpTexture = resource.GetTexture( "LeatherBump1" );
    ...
    m_Object[0].ReflectionStrength = 0.0f;

    m_Object[1].DiffuseTexture = resource.GetTexture( "Leather2" );
    m_Object[1].BumpTexture = resource.GetTexture( "LeatherBump1" );
...
BOOL CSample18::InitShaders()
{
    // Create the leather shader
    {
        ...
        if( FALSE == FrmCompileShaderProgramFromFile( "Demos/AdrenoShaders/Shaders/LeatherShader.vs", "Demos/AdrenoShaders/Shaders/LeatherShader.fs",
            &m_LeatherShader.ShaderId,
            pShaderAttributes, nNumShaderAttributes ) )
            return FALSE;

        // Make a record of the location for each shader constant
        m_LeatherShader.m_ModelMatrixId         = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_MatModel" );
        m_LeatherShader.m_ModelViewProjMatrixId = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_MatModelViewProj" );
        m_LeatherShader.m_NormalMatrixId        = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_MatNormal" );
        m_LeatherShader.m_LightPositionId       = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_LightPos" );
        m_LeatherShader.m_EyePositionId         = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_EyePos" );
        m_LeatherShader.m_AmbientLightId        = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_AmbientLight" );
        m_LeatherShader.m_SpecularColorId       = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_SpecularColor" );
        m_LeatherShader.m_BumpTextureId         = glGetUniformLocation( m_LeatherShader.ShaderId,  "g_BumpTexture" );
        m_LeatherShader.m_DiffuseTextureId       = glGetUniformLocation( m_LeatherShader.ShaderId, "g_DiffuseTexture" );
    }
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample18::Destroy()
{
    // Free shader memory
    m_LeatherShader.Destroy();
...
// Desc: 
//--------------------------------------------------------------------------------------
FRMVECTOR3 GetCameraPos18( FLOAT32 fTime )
{
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample18::DrawObject( SimpleObject18* Object )
{
    ...
    glUseProgram( m_LeatherShader.ShaderId );
    glUniformMatrix4fv( m_LeatherShader.m_ModelMatrixId, 1, FALSE, (FLOAT32*)&MatModel );
    glUniformMatrix4fv( m_LeatherShader.m_ModelViewProjMatrixId, 1, FALSE, (FLOAT32*)&MatModelViewProj );
    glUniformMatrix4fv( m_LeatherShader.m_NormalMatrixId, 1, FALSE, (FLOAT32*)&MatNormal );
    glUniform3fv( m_LeatherShader.m_EyePositionId, 1, (FLOAT32*)&Eye.v );
    glUniform3fv( m_LeatherShader.m_LightPositionId, 1, (FLOAT32*)&m_LightPos.v );
    glUniform4fv( m_LeatherShader.m_AmbientLightId, 1, (FLOAT32*)&m_AmbientLight.v );
    glUniform4fv( m_LeatherShader.m_SpecularColorId, 1, Object->SpecularColor.v );

    SetTexture( m_LeatherShader.m_DiffuseTextureId, Object->DiffuseTexture->m_hTextureHandle, 0 );
    SetTexture( m_LeatherShader.m_BumpTextureId, Object->BumpTexture->m_hTextureHandle, 1 );
