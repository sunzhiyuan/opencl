...
// Desc: Constructor
//--------------------------------------------------------------------------------------
CSample09::CSample09( const CHAR* strName ) : CScene( strName )
{
    m_hPerPixelLightingShader =	NULL;
	m_pCookTorranceLookUpTexture = NULL;
...
BOOL CSample09::InitShaders()
{
    // Create the PerPixelLighting shader
    {
        ...
        if( FALSE == FrmCompileShaderProgramFromFile( "Demos/AdrenoShaders/Shaders/CookTorrance.vs",
                                                      "Demos/AdrenoShaders/Shaders/CookTorrance.fs",
        ...
        m_hCookTorranceLookup    = glGetUniformLocation( m_hPerPixelLightingShader,  "g_sCTTexture" );
    }
...
// Desc: 
//--------------------------------------------------------------------------------------
BOOL CSample09::Initialize(CFrmFontGLES &m_Font, CFrmTexture* m_pLogoTexture)
{
    ...
    if( FALSE == resource.LoadFromFile( "Demos/AdrenoShaders/Textures/09_Textures.pak" ) )
        return FALSE;

	// get a handle to the Cook-Torrance look-up texture
	m_pCookTorranceLookUpTexture = resource.GetTexture( "CookTorrance" );
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample09::Destroy()
{
    // Free shader memory
    if( m_hPerPixelLightingShader ) glDeleteProgram( m_hPerPixelLightingShader );

	// Free texture memory
    if( m_pCookTorranceLookUpTexture ) m_pCookTorranceLookUpTexture->Release();
