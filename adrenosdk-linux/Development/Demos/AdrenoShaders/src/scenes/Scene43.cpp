...
// Desc:
//--------------------------------------------------------------------------------------
MaterialShader43::MaterialShader43()
{
    ShaderId = 0;
}
...
// Desc:
//--------------------------------------------------------------------------------------
DownsizeShader43::DownsizeShader43()
{
    ShaderId = 0;
}
...
// Desc:
//--------------------------------------------------------------------------------------
CombineShader43::CombineShader43()
{
    ShaderId = 0;
	WarpAmount = 0.0f;
}
...
// Desc:
//--------------------------------------------------------------------------------------
VOID SimpleObject43::Update( FLOAT ElapsedTime, BOOL ShouldRotate )
{
    if( ShouldRotate )
...
// Desc: Constructor
//--------------------------------------------------------------------------------------
CSample43::CSample43( const CHAR* strName ) : CScene( strName )
{
    m_nWidth = 800;
    m_nHeight = 480;
}
...
// Desc: 
//--------------------------------------------------------------------------------------
BOOL CSample43::CheckFrameBufferStatus()
{
...
// Desc: 
//--------------------------------------------------------------------------------------
BOOL CSample43::Initialize(CFrmFontGLES &Font, CFrmTexture* m_pLogoTexture)
{
    m_ShouldRotate = TRUE;
    ...
    m_UserInterface.AddFloatVariable( &m_CombineShader.WarpAmount, (char *)"Warp Amount" );
  	m_UserInterface.AddHelpKey( FRM_FONT_KEY_4, (char *)"Decrease Warp Amount" );
    m_UserInterface.AddHelpKey( FRM_FONT_KEY_5, (char *)"Increase Warp Amount" );
...
BOOL CSample43::InitShaders()
{
    // Create the material shader
    {
        ...
        if( FALSE == FrmCompileShaderProgramFromFile( "Demos/AdrenoShaders/Shaders/MaterialShader43.vs", "Demos/AdrenoShaders/Shaders/MaterialShader43.fs",
    ...
    // combine shader
    {
        ...
        if( FALSE == FrmCompileShaderProgramFromFile( "Demos/AdrenoShaders/Shaders/CombineShader43.vs", "Demos/AdrenoShaders/Shaders/CombineShader43.fs",
        ...
        m_CombineShader.WarpAmountId = glGetUniformLocation( m_CombineShader.ShaderId, "g_WarpAmount" );
   }
...
// Desc: 
//--------------------------------------------------------------------------------------
FRMVECTOR3 GetCameraPos43( FLOAT32 fTime )
{
...
VOID CSample43::Update()
{
    ...
    {
        m_CombineShader.WarpAmount -= 1.0f * ElapsedTime;
        m_CombineShader.WarpAmount = max( 0.0f, m_CombineShader.WarpAmount );
    }

    if( nButtons & INPUT_KEY_5 )
    {
        m_CombineShader.WarpAmount += 1.0f * ElapsedTime;
        m_CombineShader.WarpAmount = min(2.0f, m_CombineShader.WarpAmount );
    }
    ...
    FRMVECTOR3 LookPos = GetCameraPos43( TotalTime + 0.05f );
    m_CameraPos   = GetCameraPos43( TotalTime );
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample43::DrawObject( SimpleObject43* Object )
{
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample43::DrawCombinedScene()
{
    ...
    glUniform1f( m_CombineShader.WarpAmountId, m_CombineShader.WarpAmount );
