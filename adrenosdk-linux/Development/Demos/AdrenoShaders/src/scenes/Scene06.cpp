...
// Desc: Constructor
//--------------------------------------------------------------------------------------
CSample06::CSample06( const CHAR* strName ) : CScene( strName )
{
    m_hPerPixelLightingShader =	NULL;
	m_pBlinnPhongLookUpTexture =	NULL;
...
BOOL CSample06::InitShaders()
{
    // Create the PerPixelLighting shader
    {
        ...
        if( FALSE == FrmCompileShaderProgramFromFile( "Demos/AdrenoShaders/Shaders/IllumMap.vs",
                                                      "Demos/AdrenoShaders/Shaders/IllumMap.fs",
		...
		m_hIllumTextureSlot		  = glGetUniformLocation( m_hPerPixelLightingShader,  "IllumSampler" );
...
// Desc: 
//--------------------------------------------------------------------------------------
BOOL CSample06::Initialize(CFrmFontGLES &m_Font, CFrmTexture* m_pLogoTexture)
{
    ...
    if( FALSE == resource.LoadFromFile( "Demos/AdrenoShaders/Textures/06_Textures.pak" ) )
        return FALSE;

	// Create the anisotropic texture
	m_pBlinnPhongLookUpTexture = resource.GetTexture( "PhongLookup" );
...
// Desc: 
//--------------------------------------------------------------------------------------
VOID CSample06::Destroy()
{
    // Free shader memory
    if( m_hPerPixelLightingShader ) glDeleteProgram( m_hPerPixelLightingShader );
    // Free texture memory
	if( m_pBlinnPhongLookUpTexture )	   m_pBlinnPhongLookUpTexture->Release();
...
VOID CSample06::Render()
{
	...
	glUniform1i( m_hIllumTextureSlot, 0 );
	glBindTexture( GL_TEXTURE_2D, m_pBlinnPhongLookUpTexture->m_hTextureHandle );
