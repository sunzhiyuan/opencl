...
{
    return new CSample( "MultisampleFramebufferOGLES30" );
}
				...
				UINT32 nInternalFormat, FrameBufferObject** ppFBO )
{
    ...
    glRenderbufferStorageMultisample( GL_RENDERBUFFER, 4, nInternalFormat, nWidth, nHeight );

    // Create the depth buffer
    glGenRenderbuffers( 1, &(*ppFBO)->m_hRenderBuffer );
    glBindRenderbuffer( GL_RENDERBUFFER, (*ppFBO)->m_hRenderBuffer );
    glRenderbufferStorageMultisample( GL_RENDERBUFFER, 4, GL_DEPTH_COMPONENT24, nWidth, nHeight );
...
BOOL CSample::Initialize()
{
    ...
    // Create an offscreen Multisample FBO to render to
...
VOID CSample::Render()
{
    ...
    // Render the scene into the offscreen multisample FBO with a green background
    {
	...
	glBlitFramebuffer( 0, 0, g_pOffscreenFBO->m_nWidth, g_pOffscreenFBO->m_nHeight,
					   0, 0, g_pOffscreenFBO->m_nWidth, g_pOffscreenFBO->m_nHeight,
