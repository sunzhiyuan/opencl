...
{
    return new CSample( "FBOTextureFormatsOGLES30" );
}
...
// Must define the sampler according to the format of the texture
	...
	"	 vec3 finalColor;													\n"
	"	 finalColor.r = (texel.r<c)? cf*texel.r : (1+a)*pow(texel.r, e)-a;	\n"
	"	 finalColor.g = (texel.g<c)? cf*texel.g : (1+a)*pow(texel.g, e)-a;	\n"
	"	 finalColor.b = (texel.b<c)? cf*texel.b : (1+a)*pow(texel.b, e)-a;	\n"
...
VOID CSample::Render()
{
    ...
    // Display the offscreen FBO texture as an overlay
    {
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(FLOAT32) * 16, vQuad, GL_DYNAMIC_DRAW);
