...
//--------------------------------------------------------------------------------------
// File: CompressedGeometryVS.hlsl
...
PixelShaderInput main(VertexShaderInput input)
{
	...
	float4 vPos = float4(input.vPositionOS.xyz * 0.5 + 0.5, 1.0);//unmap from short range
	vPos = mul(matCT, vPos);//unmap from (0,1)
    ...
    float3 vNormalES = mul((float3x3)matNormal, vNormal).xyz;
