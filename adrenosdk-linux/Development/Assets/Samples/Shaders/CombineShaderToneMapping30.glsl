...
void main()
{
    vec4 ScreenPixel = texture( g_SharpTexture, g_TexCoord );
    vec4 Bloom = texture( g_BloomTexture, g_TexCoord );
    
	// Current adaption level of luminance (1x1 texture)
	float AdaptedLum = clamp( texture( g_CurrentLuminance, vec2( 0.0, 0.0 ) ).r, 0.1, 0.7 );
	...
	g_FragColor = clamp(vec4(finalColor.r, finalColor.g, finalColor.b, 1.0), 0.0, 1.0);
	*/
	
	g_FragColor = clamp(vec4(sample_bloom.r, sample_bloom.g, sample_bloom.b, 1.0), 0.0, 1.0);
}
